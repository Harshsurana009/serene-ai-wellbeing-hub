...
import { createConversation, getConversation, endConversation } from '../tavusApi';
...
describe('Tavus API Service', () => {
  describe('createConversation', () => {
    it('should create a conversation successfully', async () => {
      const mockApiResponse = {
        id: 'conv_abc',
        status: 'pending',
        join_url: 'https://meet.tavus.io/session/abc',
        created_at: '2025-04-27T10:00:00Z',
        expires_at: '2025-04-27T10:30:00Z',
      };
      ...
      expect(fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: expect.objectContaining({
            'x-api-key': '705c82d622284bf4aaf2d9cdbd33e144',
            'Content-Type': 'application/json',
          }),
          body: expect.stringContaining('"conversation_name":"Wellness Check Session with John Doe"'),
        })
      );
      expect(result).toEqual({
        id: 'conv_abc',
        status: 'pending',
        joinUrl: 'https://meet.tavus.io/session/abc',
        createdAt: '2025-04-27T10:00:00Z',
        expiresAt: '2025-04-27T10:30:00Z',
      });
    });
    ...
    it('should handle API error during creation', async () => {
     ...
     it('should handle specific "maximum concurrent conversations" error', async () => {
      const mockErrorResponse = { message: 'Account has reached the maximum concurrent conversations limit.' };
      (fetch as jest.Mock).mockResolvedValueOnce({
        ok: false,
        status: 400, // Or whatever status Tavus returns for this
        statusText: 'Bad Request',
        json: async () => mockErrorResponse,
      });

      await expect(createConversation(mockParams)).rejects.toThrow(
        'Account has reached the maximum concurrent conversations limit.. Please try again later or end existing sessions.'
      );
      expect(fetch).toHaveBeenCalledTimes(1);
    });
  ...
  describe('getConversation', () => {
    ...
    it('should use fallback mock response on get conversation error', async () => {
      ...
      expect(result.joinUrl).toBe(`https://meet.tavus.io/session/${conversationId}`);
      expect(console.warn).toHaveBeenCalledWith("Using fallback mock response");
    });
  });
